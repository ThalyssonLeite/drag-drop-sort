...
export class DragAndDropListLib {
  /*        __----------------------------__       */
  ...
  private drag (event) {
    ...
    if (mouseY < this.limits.top && mouseY < this.limits.bottom) {
      ...
      this.$boxClone.style.borderRadius = `${this.containerBounds.topRadius} 0 0 `
      ...
      this.$boxClone.style.top = `${mouseY}px`;
      this.$boxClone.style.borderRadius = `0 0 ${this.containerBounds.bottomRadius}`

      //If the box is inside of the container TOP or BOTTOM, make it follow the mouse movement
    } else  {
      this.$boxClone.style.top = `${mouseY}px`;
      this.$boxClone.style.borderRadius = '0';
    }
